...
class densenetlike(nn.Module):
    ...
    def __init__(self, num_chunks = 3, growth_rate = 12, kernel_size = 3,
                    num_layers = 3, bottleneck_width = 8, compression = 0.40):
        ...
        self.conv = AutoConv2d_BN(3, in_channels, kernel_size = 5)
        ...
        self.dense = DenseBlock(input_channels = self.out_channels[-1],
                                kernel_size = kernel_size,
                                growth_rate = growth_rate,
                                num_layers = num_layers,
                                bottleneck_width = bottleneck_width)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(self.out_channels[-1] + growth_rate * num_layers, 10)
    
    def forward(self, x):
        x = self.conv(x)
        for _, chunk in enumerate(self.chunks):
            x = chunk(x)
        x = self.dense(x)
