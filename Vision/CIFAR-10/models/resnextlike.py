...
class resnextlike(nn.Module):
    ...
    def __init__(self, in_channels = 128, cardinality = 64, bottleneck_width = 4):
        super().__init__()
        self.conv1 = nn.Conv2d(3, in_channels, 5, padding = 2)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.block1 = ResNeXtBlock(in_channels, 2 * in_channels, down_stride = 2, 
                                    cardinality = cardinality, 
                                    bottleneck_width = bottleneck_width // 2)
        self.block2 = ResNeXtBlock(2 * in_channels, 2 * in_channels,
                                    cardinality = cardinality, 
                                    bottleneck_width = bottleneck_width // 2)
        self.block3 = ResNeXtBlock(2 * in_channels, 4 * in_channels, down_stride = 2, 
                                    cardinality = cardinality, 
                                    bottleneck_width = bottleneck_width)
        self.block4 = ResNeXtBlock(4 * in_channels, 4 * in_channels, 
                                    cardinality = cardinality, 
                                    bottleneck_width = bottleneck_width)
        self.block5 = ResNeXtBlock(4 * in_channels, 8 * in_channels, down_stride = 2,
                                    cardinality = cardinality, 
                                    bottleneck_width = 2 * bottleneck_width)
        self.block6 = ResNeXtBlock(8 * in_channels, 8 * in_channels, 
                                    cardinality = cardinality, 
                                    bottleneck_width = 2 * bottleneck_width)
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout2d(0.2)
        self.fc = nn.Linear(8 * in_channels, 10)
